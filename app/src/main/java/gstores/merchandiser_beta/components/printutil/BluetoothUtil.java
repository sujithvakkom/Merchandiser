package gstores.merchandiser_beta.components.printutil;
    ...
    public static boolean connectBlueTooth(Context context) {
        if (bluetoothSocket == null) {
            ...
            if (!getBTAdapter().isEnabled()) {
                Toast.makeText(context, R.string.bluetooth_not_enabled, Toast.LENGTH_SHORT).show();
                return false;
            }
            BluetoothDevice device;
            if ((device = getDevice(getBTAdapter())) == null) {
                Toast.makeText(context, R.string.bluetooth_not_device, Toast.LENGTH_SHORT).show();
                return false;
            }
